...
function restart(update = true) {
    ...
    game.totalJumps = 0;
    game.gameEnd = false;
    game.solution = game.solution || [0, 2, 5, 7, 11];
    game.isFirstGame = true;
...
function updateFilling() {
    for (let i = 1; i < 6; i++) {
        for (let j = 1; j < 6; j++) {
            ...
            let filling = 0.01;
            if (!game.isFirstGame && game.solution.includes(position)) filling = 0.12;
...
function check() {
    ...
    for (let i = 0; i < boardCopy.length; i++) {
        ...
        game.solution.forEach(v => foundSolution = foundSolution && boardCopy[i + v]);
        if (foundSolution) {
            isOk = true;
            break;
        }
