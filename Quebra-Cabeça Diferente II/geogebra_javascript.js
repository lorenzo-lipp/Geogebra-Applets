...
class Piece {
    constructor(id, order, angle) {
        ...
        setLayer(`up${this.id}`, order.findIndex(v => v === UP));
        setLayer(`middle${this.id}`, order.findIndex(v => v === MIDDLE));
        setLayer(`down${this.id}`, order.findIndex(v => v === DOWN));
        setLayer(`left${this.id}`, order.findIndex(v => v === LEFT));
        setLayer(`center${this.id}`, order.findIndex(v => v === CENTER));
        setLayer(`right${this.id}`, order.findIndex(v => v === RIGHT));
    }
    ...
    select() { 
        ...
        for (let part of this.parts) setLayer(part, getLayer(part) + 4);
        this.selected = true;
    }
...
let game = {
    pieces: Array(14),
    correctOrder: [CENTER, DOWN, RIGHT, MIDDLE, LEFT, UP],
    board: [
        Array(3).fill(null),
        Array(3).fill(null),
        Array(3).fill(null)
...
function newGame() {
    let positions = shuffle(game.positions);
    let vertical = shuffle([UP, DOWN, MIDDLE]);
    let horizontal = shuffle([RIGHT, LEFT, CENTER]);
