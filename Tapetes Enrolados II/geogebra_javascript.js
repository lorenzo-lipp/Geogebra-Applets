var g = ggbApplet
var colors = ["Laranja", "Verde", "Rosa", "Amarelo", "Roxo", "Vermelho", "Azul"]
var dict = {"Laranja": "L", "Rosa": "L", "Verde": "L", "Amarelo": "R", "Azul": "R", "Vermelho": "R", "Roxo":"R"}
var order = []
var layer = 1
// Ordem inicial do jogo
var gameList = [2, 6, 4, 1, 3, 7, 5]
...
function update(color) {
    ...
    if (value == 1) {
        ...
        g.setLayer("Tapete" + color + "5", 0)
    }
    else if (value > 1) {
        g.setFixed("Box" + color + "1", 1, 0)
        if (value == 5 && order[order.length - 1] == color) {g.setFixed("Box" + color + "2", 1, 1)}
    }
}
...
function validate() {
    let answer = []
    for(var x in colors) {answer.push(g.getLayer("Tapete" + colors[x] + "5"))}
    for (var x in answer) {
...
function newGame(oldGameList) {
    while(sameList(oldGameList, gameList)) {
        let leftList = shuffle([2,4,6])
        let rightList = shuffle([1,3,5,7])
        gameList = leftList
...
function reset() {
    ...
    order = []
    for (var x = 0; x < 7; x++) {
        g.setLayer("Tapete" + colors[x] + "5", 0)
