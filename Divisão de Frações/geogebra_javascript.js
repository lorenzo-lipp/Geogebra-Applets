...
class Game {
    ...
    get result() { 
        if (this.gameType === 1) {
            return (this.initValues[0] * this.initValues[2]) / (this.initValues[1] * this.initValues[3]); 
        } else {
    ...
    nextStep() {
        ...
        if (solutionStep === 0) {
            ggbApplet.setTextValue("texto1", `\\text{Inverta a fração } \\frac{${num2}}{${den2}} \\text{ e}`);
            ggbApplet.setTextValue("text14", `multiplique os numeradores e os denominadores das duas frações.`);
        } else if (solutionStep === 1) {
            ggbApplet.setTextValue("text17", `${num1} \\times ${den2} = ?                  ${den1} \\times ${num2} = ?`);
        } else if (solutionStep === 2) {
            ggbApplet.setTextValue("text17", `${num1} \\times ${den2} = ${num1 * den2}                  ${den1} \\times ${num2} = ${den1 * num2}`);
        } else {
            ggbApplet.setTextValue("text20", `\\frac{${num1}}{${den1}} \\div \\frac{${num2}}{${den2}} = \\frac{${num1}}{${den1}} \\times \\frac{${den2}}{${num2}} = \\frac{${num1 * den2}}{${num2 * den1}}`);
        }
